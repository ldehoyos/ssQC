function getSagittalPNG(dirOS,subjID, slice)
...
%   slice is the saggital slice you want to output.
%   subjID is the name of the subject.

% Read the image.
fID= [dirOS, subjID, '/orig.mgz'];
...
h(1)= imshow(CT_S, 'DisplayRange', [0 200]);
text('units','pixels','position',[3 -9],'BackgroundColor','r','fontsize',12,'string',subjID,'Interpreter','none')
hold off;
axis off;
saveas(gca,char(strcat(dirOS, 'Sagittal_View_',num2str(slice), '_Subj_', subjID,'.png')),'png')
close all;
